...
def main():
    ...
    for line in content.splitlines():
        ...
        if current_module:
            ...
            m_item = re.search(r"^[*-] \[[xX ]\] .*?`([a-zA-Z0-9_]+)`", line)
    ...
    for mod_name, funcs in modules.items():
        if not funcs:
            continue
        prefix = "PAX_TODO.md".split("_")[0].lower()
        orig_mod_name = mod_name.replace("zero_pax", prefix)
        mod_implemented = sum(
