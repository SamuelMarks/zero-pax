...
The `ml-framework-snapshots` tool verifies that `zero-pax` accurately replicates the public API signatures, parameters, and defaults of the real `pax` framework.

**To run the checker against the current codebase:**

```bash
ml_framework_snapshots check pax ~/repos/zero-pax/src \
    --reference-prefix pax \
    --target-prefix zero_pax
```

**What it does:**
- It statically analyzes all Python modules in `zero-pax/src`.
- It maps the target namespace (`zero_pax.*`) back to the reference framework (`pax.*`).
...
```bash
ml-switcheroo-ir compliance ~/repos/zero-pax/src
```
